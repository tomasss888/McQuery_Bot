...
function setupEmbed(ip, client, interaction) {
    ...
    getMcData(hostname)
        .then(async (result) => {
            ...
            var statusEmbed = await manageEmbed(client, interaction, result)
            config.channelID.embed = statusEmbed.id;
        
            // Initiates Interval to keep updating created embed
            await interaction.guild.channels.cache.get(config.channelID.infoChannel).send({ embeds: [statusEmbed] }).then((msg) => {
...
async function updateEmbed(client, interaction, callback) {
    ...
    getMcData(hostname).then(async (result) => {
        embed = await manageEmbed(client, interaction, result);
        callback(await embed);
    })
}

// Updates bot status, channels and embed
function manageEmbed(client, interaction, json) {
